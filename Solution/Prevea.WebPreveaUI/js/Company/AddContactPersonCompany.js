var AddContactPersonCompany = kendo.observable({
    ...
    setKendoUIWidgets: function () {
        $($("#" + this.btnCancelId)).on("click", function () {
            AddContactPersonCompany.goToDetailCompany();
        });

        $($("#" + this.btnValidateId)).on("click", function (e) {
            var errors = AddContactPersonCompany.validateForm();
    ...
    goToContactPersonsCompany: function () {
        var params = {
            url: "/Company/ContactPersonsCompany"
        };
        GeneralData.goToActionController(params);
    },
